...
// gameLogic.js หรือ AI.js (ขึ้นอยู่กับโครงสร้างของโปรเจกต์คุณ)

// กองไพ่ทั้งหมด
let deck = [
  { suit: 'Hearts', value: '2' }, { suit: 'Hearts', value: '3' }, { suit: 'Hearts', value: '4' }, 
  { suit: 'Spades', value: '5' }, { suit: 'Spades', value: '6' }, { suit: 'Diamonds', value: '7' },
  { suit: 'Clubs', value: '8' }, { suit: 'Diamonds', value: '9' }, // และอื่น ๆ
];

// ฟังก์ชันสุ่มไพ่จากกอง
function drawCardFromDeck() {
  // ตรวจสอบว่ากองไพ่ยังมีอยู่หรือไม่
  if (deck.length === 0) {
  ...
  const drawnCard = deck.splice(randomIndex, 1)[0]; // เอาไพ่ที่ถูกเลือกออกจากกอง

  return drawnCard; // คืนค่าไพ่ที่สุ่ม
}

// ฟังก์ชันให้ AI เล่นไพ่
function aiPlayTurn() {
  const drawnCard = drawCardFromDeck(); // AI เลือกไพ่จากกอง
  if (drawnCard) {
    console.log("AI selected card:", drawnCard); // แสดงผลไพ่ที่ AI เลือก
    // ทำอะไรก็ตามกับไพ่ที่เลือก เช่น เล่นไพ่ หรือเก็บไพ่
  } else {
    console.log("No cards left in the deck.");
  }
}

// ตัวอย่างการเรียกใช้ในเกม
aiPlayTurn();  // AI เลือกไพ่จากกอง
